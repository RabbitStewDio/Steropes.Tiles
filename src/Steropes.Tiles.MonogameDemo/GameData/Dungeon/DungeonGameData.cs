...
using Steropes.Tiles.MonogameDemo.GameData.Dungeon.Simple;
...
namespace Steropes.Tiles.MonogameDemo.GameData.Dungeon
{
  ...
  public class DungeonGameData
  {
    ...
    public DungeonGameData()
    {
      ...
      Map = new DungeonMap(MapWidth, MapHeight, Rules);
    ...
    public List<IItem> QueryItems(int x, int y, List<IItem> collector)
    {
      return containerSrv.Contents(containerRefs.From(new MapCoordinate(x, y)), collector);
    }

    public IEnumerable<IItem> QueryItems(int x, int y)
    {
      return containerSrv.Contents(containerRefs.From(new MapCoordinate(x, y)));
    }
