...
{
  internal class RenderCallbackBase<TTile, TContext> : IRenderCallback<TTile, TContext>
  {
  ...
  public class RenderStackingTest
  {
    ...
    public void RenderAccess_With_Cache_ViewPort()
    {
      ...
      var matcher = Substitute.For<ITileMatcher<RenderedTile, RenderedContext>>();
          ...
          var cache =
            PlotOperations.FromContext(rc).Create(matcher)
              .WithCache()
              .ForViewport()
              .WithRenderer(renderer).Build();
