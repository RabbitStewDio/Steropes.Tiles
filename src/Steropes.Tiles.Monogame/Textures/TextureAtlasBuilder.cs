...
namespace Steropes.Tiles.Monogame.Textures
{
  ...
  public class TextureAtlasBuilder
  {
    class TreeNode
    {
      ...
      public TreeNode Insert(ITexture tex, int padding)
      {
        if (!Leaf)
        {
      ...
      public ITexture Harvest(RenderTarget2D targetTexture)
      {
        if (Texture != null)
        {
          ...
          b.Draw(Texture.Tex2D, CellBounds, Texture.Bounds, Color.White);
          b.End();

          g.SetRenderTarget(null);

          return new TileTexture(Texture.Name, targetTexture, CellBounds);
        }
    ...
    public bool Insert(ITexture tile, out ITexture result)
    {
      if (!IsValid(tile))
      {
        result = tile;
    ...
    static bool IsValid(ITexture tex)
    {
      return tex?.Tex2D != null;
    }
  }
}