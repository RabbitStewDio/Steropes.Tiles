...
{
  /// <summary>
  ///  This tile selector selects all ocean tiles that have a river outlet. For this,
  ///  self-match checks for oceans, and neighbour match checks for land tiles that 
  ///  have the river-flag set. This matcher works in addition and after the normal 
  ///  ocean tile matcher.
  /// </summary>
  /// <para>
  ///  River matching is a combination of a (separate) CardinalTileSelector matching any tile
  ///  with a river-flag set and this shore-matcher. We assume that the map is smart enough 
  ///  to not flag ocean tiles as having a river. The self-matcher should strictly check 
  ///  whether a tile is flagged as having a river. The neighbour-matcher should check whether 
  ///  any of the matched tiles is either a river or a water tile (lake, ocean).
  /// </para>
  ...
  public class RiverOutletTileSelector<TRenderTile, TContext> : BaseTileSelector<TRenderTile, TContext>
  {
      ...
      : base(matcher, gridNavigator, contextProvider)
    {
      ...
      this.northTile = outletRegistry.Find(prefix, CardinalIndex.North);
      this.eastTile = outletRegistry.Find(prefix, CardinalIndex.East);
      this.southTile = outletRegistry.Find(prefix, CardinalIndex.South);
      this.westTile = outletRegistry.Find(prefix, CardinalIndex.West);
    }
