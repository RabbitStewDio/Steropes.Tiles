...
{
  internal class WrapAroundNavigator<T> : IMapNavigator<T> where T : struct
  {
    readonly int lowerX;
    readonly int lowerY;
    readonly int deltaX;
  ...
  internal class WrapAroundVertical<T> : IMapNavigator<T> where T : struct
  {
    readonly int lowerY;
    readonly int deltaY;
    readonly IMapNavigator<T> parent;
  ...
  internal class WrapAroundHorizontal<T> : IMapNavigator<T> where T : struct
  {
    readonly int lowerX;
    readonly int deltaX;
    readonly IMapNavigator<T> parent;
  ...
  {
    public static IMapNavigator<T> Wrap<T>(this IMapNavigator<T> parent, Range? x, Range? y) where T: struct
    {
    ...
    public static IMapNavigator<T> Wrap<T>(this IMapNavigator<T> parent, int? x, int? y) where T: struct
    {
      if (x.HasValue && y.HasValue)
      {
        return new WrapAroundNavigator<T>(parent, x.Value, y.Value);
